<?php
    ...
    class WorkoutLogController extends Controller
    {
        ...
        {
            $user = Auth::user();
            $sessions = WorkoutSession::where('user_id', $user->id)->with(['sets.exercise'])->orderByDesc('trained_at')->get()->map(fn ($session) => $this->formatSession($session));
        ...
        public function storeSession (Request $request): JsonResponse
        {
            ...
            return response()->json([
                ...
                'data'    => $this->formatSession($session->load('sets.exercise')),], 201);
        }
        ...
        public function storeSets (Request $request, WorkoutSession $session): JsonResponse
        {
            ...
            $exercises = Exercise::whereIn('external_id', $externalIds)->pluck('id', 'external_id');
            ...
            foreach ($data['sets'] as $setData)
            {
                ...
                {
                    return response()->json([
                        'error' => "Exercise not found: {$setData['external_id']}",], 422);
                }
        ...
        {
            $user = Auth::user();
            abort_if($session->user_id !== $user->id, 403, 'Forbidden');
        }
