    ...
    class MealPlanService
    {
        ...
        private function selectFoodsForUser (UserProfile $profile): array
        {
            ...
            $preferred = $allFoods->filter(function (Food $food) use ($preferences, $excluded, $isVegetarian, $isVegan) {
                ...
                return $matchesPref && !$this->isExcluded($name, $excluded) && !($isVegetarian && $this->matchesKeywords($name, self::MEAT_KEYWORDS)) && !($isVegan && $this->matchesKeywords($name, [
                            'egg',
                            'dairy',
                            'milk',
                            'cheese',
            ...
            if ($preferred->count() < 6)
            {
                $supplemental = $allFoods->filter(function (Food $food) use ($preferred, $excluded, $isVegetarian, $isVegan) {
                    if ($preferred->contains('id', $food->id))
                        return false;
                    $name = strtolower($food->name);
                    return !$this->isExcluded($name, $excluded) && !($isVegetarian && $this->matchesKeywords($name, self::MEAT_KEYWORDS)) && !($isVegan && $this->matchesKeywords($name, [
                                'egg',
                                'dairy',
                                'milk',
                                'cheese',
