...
const chipStyles = StyleSheet.create({
  base:        {paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: INNER, borderWidth: 1, borderColor: BORDER},
...
const statStyles = StyleSheet.create({
  box:   {flex: 1, backgroundColor: INNER, borderRadius: 12, padding: 14, alignItems: 'center', gap: 3},
...
const settingsRowStyles = StyleSheet.create({
  ...
  iconBox:        {width: 36, height: 36, borderRadius: 10, backgroundColor: INNER, alignItems: 'center', justifyContent: 'center'},
...
const macroRowStyles = StyleSheet.create({
  row:     {flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 9, borderBottomWidth: 1, borderColor: BORDER},
...
function Chip({label, variant = 'default'}: {label: string; variant?: 'default' | 'green' | 'red'}) {
  if (variant === 'green') {
    return (
...
function MacroRow({label, value, unit, color, isLast = false}: {
  label: string; value: number; unit: string; color: string; isLast?: boolean;
}) {
  return (
                ...
                {(profileData.food_preferences?.length ?? 0) > 0 && (
                  <View style={s.chipSection}>
                    ...
                    <View style={s.chipRow}>
                      {profileData.food_preferences.map(p => <Chip key={p} label={p} variant="green" />)}
                    </View>
                  </View>
                )}
...
const s = StyleSheet.create({
  ...
  goalBadge:     {flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#052E16', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: '#166534'},
  ...
  workoutBadge:     {backgroundColor: '#1A2A1E', paddingHorizontal: 14, paddingVertical: 9, borderRadius: 10, borderWidth: 1, borderColor: '#166534'},
